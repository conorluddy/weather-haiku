...
async function getWeather(position) {
    ...
    const ts2 = new (0, _typeShuffle.TypeShuffle)(subTextElement);
    try {
        const haikuResponse = await (0, _utils.loadHaiku)(latitude, longitude);
        const haikuJson1 = await haikuResponse.json();
        document.body.classList.remove("loading");
        coordContainer.innerHTML = `${latitude}, ${longitude}`;
        updateHaikuUi(haikuJson1.haiku);
        updateWeatherUi(haikuJson1.weather);
        ts.trigger("fx3");
        ts2.trigger("fx3");
    } catch (error) {
        const haikuContainer = document.querySelector(".haiku");
        haikuContainer.innerHTML = "Error getting weather, probably don't have permission to get your location.";
        console.error(error);
    }
}
...
parcelHelpers.export(exports, "loadHaiku", ()=>loadHaiku);
...
},{}]},["gHDMI","ebWYT"], "ebWYT", "parcelRequireb3e5")
